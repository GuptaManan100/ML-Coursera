...
J = J + (lambda/(m+m) * (sum(sum(regTheta1.*regTheta1)) +sum(sum(regTheta2.*regTheta2))));
...
for(i = 1:m)
  ...
  del2 = (Theta2'*del3').*sigmoidGradient([0;ztem2']);
  
  del2 = del2(2:end);
  
  delta1 = delta1 + del2*atem1;
  delta2 = delta2 + del3'*atem2;
end;
